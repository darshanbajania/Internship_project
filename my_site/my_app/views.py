    ...
    if request.session.has_key('is_logged_in'):  # checking for sessions
        if request.user.is_authenticated:
            if request.user.username == "admin":  # check wheter the use is admin or not
                ...
                return redirect('my_app:admin_profile')
            ...
            for i in range(1, user_count+1):
                ...
                # print(request.user.username)
                    p = str(b)
                    # checking if current user name matches with the mentors in database
                    if(p == request.user.username):
                        # print(user.user)
                        # print(user.name)
...
def Proposals_View(request):
    if request.session.has_key('is_logged_in'):  # checking for sessions
        ...
        if request.method == "POST":
            ...
            base_url = reverse('my_app:full_proposal')
            # creating a string of dictionary
            query_string = urlencode({'category': proposal_number})
        ...
        if(p == request.user.username):  # checking if current user name matches with the mentors in database
            if user.prop_no == "def":
                ...
                c = len(test)
                count = 0
                bsc = Proposal.objects.filter(
                    ids="abc")  # initialising with None
            else:
                c = int(user.prop_no)  # it is the no. of proposals
                count = 0
                bsc = Proposal.objects.filter(
                    ids="asd")  # initializing with None
            # gettting the no. of proposals assigned to current mentor
            for ele in list(user.propsl_list.split(' ')):
                if count < c:
                    # filtering the proposals mentioned in proposal_list
                    mentor_proposals = Proposal.objects.filter(ids=str(ele))
                    bsc = bsc | mentor_proposals  # cascading the proposals in a query list

        # print(bsc)
        # create a paginator having 12 items per page
        pagin = Paginator(bsc, 12)
        page_number = request.GET.get('page')  # get te value of next page
        # print(pagin.page(1))
        # get the contents of next page
        paginator = pagin.get_page(page_number)
...
def Total_Proposal_View(request):
    if request.session.has_key('is_logged_in'):  # checking for sessions
        ...
        if request.method == "POST":
            proposal_number = request.POST.get('proposal')
            # getting the url for displaying full proposal
            base_url = reverse('my_app:full_proposal_admin')
            # creating a string of dictionary
            query_string = urlencode({'category': proposal_number})
            # passing the base_url and query from this page to url
            url = '{}?{}'.format(base_url, query_string)
        ...
        all_proposals = Proposal.objects.filter(
            ids='ds')  # initializing query set with None

        for i in range(0, 495):
            propsal_temp = Proposal.objects.filter(
                ids=str(i))  # filtering all the proposals
       ...
       # print(all_proposals)
        # create a paginator having 12 items per page
        pagin = Paginator(all_proposals, 12)
        # page1 = pagin.page(1)
...
def Admin_view(request):
    if request.session.has_key('is_logged_in'):
        ...
        mentors_list = Mentors.objects.filter(
            name='ds')  # initializing query set with None
        length = len(skills)
        for i in range(0, length):
            # list having name of Mentor
            name = list(proposal_list.items())[i][0]
            new_mentor_name = all_functions.convert_space_to_us(
                name)  # replacing space with underscore
            user = Mentors.objects.filter(
        ...
        if request.method == "POST":
            for i in mentors_list:  # after allocate proposals is clicked
                ...
                # getting proposal value from the form
        ...
        print("success")
        # passing the mentors list to the admin page
...
def Full_Proposal_view(request):
    if request.session.has_key('is_logged_in'):
        # after getting the category value from
        category_id = request.GET.get('category')
        # filtering the particular proposal
